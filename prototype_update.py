...
if end == "x":
    kadenz = input(
        "Wähle eine und bestätige mit der Zahl\n1: (1, 4, 5, 6, 4, 5, 1)\n2: (1, 4, 5, 1)\n3: (2, 4, 5, 1)\n4: (1, 6, 4#, 5, 1)\n5: (i, IIIb, iv, V, i)\nEingabe: ")
    if kadenz == "1":
        progression = [['C', 'E', 'G'], ['F', 'A', 'C'], ['G', 'B', 'D'], ['A', 'C', 'E'], ['F', 'A', 'C'],
                       ['G', 'B', 'D', 'F'], ['G', 'E', 'C']]
    if kadenz == "2":
...
for i_chord in progression:
    for i in range(len(i_chord) - 1):
        chord_intervalls.append(str(intervall(i_chord[i], i_chord[
            i + 1])))  # Uses intervall function to create chord intervalls for each input group and adds the intervalls to chord_intervalls
    progression_intervalls.append(
        chord_intervalls)  # Appends the intervall combination of each chord to the progression_intervalls list
...
for i in range(len(progression_intervalls)):
    ...
    chord_data.append(chord_names_dict[tuple(progression_intervalls[i])])
...
scale = []
all_scale = []
for i_root in chord_data:  # i_root ist die root-note der eingegebenen Akkorde
    for i_all in all_notes:  # i_all sind alle gegebenen Noten
        scale.append(
            str(intervall(i_root[0], i_all)))  # fügt das intervall von einer root-note zu allen gegebenen hinzu
    uni_scale = list(map(int, list(
        set(scale))))  # nachdem alle intervall in scale = [] gespeichert wurden, filtert es mit einem set alle mehrfachen raus, wandelt alle intervalle mit map in int um, um sie zu ordnen
    uni_scale.sort()  # hier wird geordnet
    all_scale.append(list(map(str,
                              uni_scale)))  # hier wird wieder alles in str umgewandelt und die skala(aus intervallen) einer liste zugefügt
...
scale_dict = {("0", "2", "4", "5", "7", "9", "11"): ["Ionisch", ("1", "2", "3", "4", "5", "6", "7")],
              ...
              ("0", "2", "3", "5", "7", "8", "10", "11"): ["Melodisch Moll",
                                                           ("1", "2", "3b", "4", "5", "6b", "7b", "7")]
...
for i_index, i_scale in enumerate(all_scale):
    for key, value in scale_dict.items():  # hier wird durch das obige scale dict mit .items() mit den jeweiligen key und value iteriert
        if all(i in key for i in
               i_scale):  # all() gibt True, wenn alle booleans darin True sind. i sind hier die ermittelten intervalle des scale estimators. dann wird überprüft ob i in dem key (der intervall-struktur der modis) is und gibt einen bool aus.
            scale_name.append([chord_data[i_index][0], value[0],
                               value[1]])  # wenn alle gegebenen intervalle in einem modus auch vorkommen ...
...
chord_root_to_root = []
for i_chord_root in chord_data:
    for i_chord in chord_data:
...
for i_scale in scale_name:  # scale_name besitzt die root-notes (index 0) und die möglichen Skalen (index 1) dazu.
    for i_root, i_root_to_root in zip(chord_data,
                                      chord_root_to_root):  # zip verbindet die root-notes mit den intervallen zwischen den root-notes
        if i_root[0] == i_scale[
            0]:  # wenn die root-note der chords mit der root-note der möglichen Skalen übereinstimmt...
            for i_root_to_root_run in i_root_to_root:  # ...wird für jedes intervall (chord-root-notes untereinander) der index in der jeweiligen Skala vermerkt
                scale_index_run.append(scale_dict_swapped[i_scale[2]].index(
                    i_root_to_root_run))  # i_scale[1] is der name der Skala und scale_dict_swapped hat den key (name der skala) und den value (struktur des modus)
...
for i_step in root_step:
    for i_s, i_ch in zip(i_step[1], chord_data):
        if i_ch[-1] == 0:
